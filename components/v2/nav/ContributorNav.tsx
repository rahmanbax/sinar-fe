...
import { LayoutGrid, Database, X } from 'lucide-react';
...
const navItems = [
    ...
    {
        name: 'Data Saya',
        href: '/v2/contributor/data-saya',
        icon: Database,
    },
];

interface ContributorNavProps {
    isOpen?: boolean;
    onClose?: () => void;
}

const ContributorNav = ({ isOpen = false, onClose }: ContributorNavProps) => {
        ...
        <>
            ...
            <aside className={`fixed md:static inset-y-0 left-0 z-50 w-[240px] md:w-[240px] bg-white border-r border-gray-100 flex flex-col py-4 shrink-0 transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="flex flex-col gap-2 m-4 space-y-1">
                    {navItems.map((item) => {
                        const isActive = pathname === item.href || (item.href !== '/v2/contributor' && pathname.startsWith(item.href));

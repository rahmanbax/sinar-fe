        ...
        try {
            ...
            if (!res.error) {
                const transformed = res.data?.map((item: any, idx: number) => ({
                    ...
                    province_name: item.province_name || item.province?.name || item.toponym?.province_name || item.toponym?.province?.name || item.province_id || item.toponym?.province_id || "-",
                    regency_name: item.regency_name || item.regency?.name || item.city_name || item.city?.name || item.toponym?.regency_name || item.toponym?.regency?.name || item.toponym?.city_name || item.toponym?.city?.name || item.regency_id || item.toponym?.regency_id || "-", 
    ...
    return (
        <div className="flex flex-col gap-6">
            <div className="flex items-center gap-3">
                    <button onClick={() => router.back()} className="p-2 hover:bg-gray-100 rounded-lg text-navy-900 transition-colors"><ChevronLeft size={24} /></button>

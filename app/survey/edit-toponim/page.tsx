...
const PreviewMap: React.FC<PreviewMapProps> = ({ isEditing, geometriType, snappingEnabled, drawnPoints, onPointsChange, savedGeometry, onClearSaved, onSave, onUndo }) => {
    ...
    const handleMouseMove = useCallback(
        (e: MapLayerMouseEvent) => {
            if (isEditing) setCursorPosition([e.lngLat.lng, e.lngLat.lat]);
        },
        [isEditing],
    );
    ...
    if (drawnPoints.length > 0) {
        drawnPoints.forEach((point, idx) => {
            currentDrawingGeoJson.features.push({ type: "Feature", properties: { id: idx }, geometry: { type: "Point", coordinates: point } });
        });
...
const EditToponimContent = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (genericElement) payload.generic_element = genericElement;
            if (specificElement) payload.specific_element = specificElement;
                    ...
                    <div className="space-y-4">
                        <Collapsible open={openSpasial} onOpenChange={setOpenSpasial}>
                            ...
                            <CollapsibleContent className="mt-4 ml-6 space-y-4">
                                {!isEditingDraft ? (
                                    <Button variant="outline" className="w-full border-blue-500 text-blue-600 hover:bg-blue-50" onClick={() => setIsEditingDraft(true)}>

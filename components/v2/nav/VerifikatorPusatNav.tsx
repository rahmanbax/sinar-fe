...
import { LayoutGrid, Database, Users, X, Map, FileCheck } from 'lucide-react';

const navItems = [
    {
        name: 'Dashboard',
        href: '/v2/verifikator-pusat',
        icon: LayoutGrid,
    },
    {
        name: 'Data Penelaahan',
        href: '/v2/verifikator-pusat/data-penelaahan',
        icon: Database,
    },
    {
        name: 'Data Provinsi',
        href: '/v2/verifikator-pusat/data-provinsi',
        icon: Map,
    },
    {
        name: 'Data Rekomendasi',
        href: '/v2/verifikator-pusat/data-rekomendasi',
        icon: FileCheck,
    },
    {
        name: 'Tim Saya',
        href: '/v2/verifikator-pusat/tim-saya',
        icon: Users,
    },
];

interface VerifikatorPusatNavProps {
    isOpen?: boolean;
    onClose?: () => void;
}

const VerifikatorPusatNav = ({ isOpen = false, onClose }: VerifikatorPusatNavProps) => {
        ...
        <>
            ...
            <aside className={`fixed md:static inset-y-0 left-0 z-50 w-[240px] md:w-[240px] bg-white border-r border-gray-100 flex flex-col py-4 shrink-0 transition-transform duration-300 md:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
                ...
                <nav className="flex flex-col gap-2 m-4 space-y-1">
                    {navItems.map((item) => {
                        const isActive = item.href === '/v2/verifikator-pusat'
                            ? pathname === item.href
...
export default VerifikatorPusatNav;

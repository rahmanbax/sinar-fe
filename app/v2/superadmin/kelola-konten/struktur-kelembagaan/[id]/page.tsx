...
import { useOrgDetail, useOrgList, useUpdateOrgNodeMutation } from "@/hooks/useCms";
...
const EditStrukturPage = () => {
    ...
    const { data: response, isLoading } = useOrgDetail(token, id);
    const { data: listResponse } = useOrgList(token);
    const node = response?.data;
    // Exclude self dari pilihan parent (hindari circular)
    const allNodes = (listResponse?.data ?? []).filter((n) => n.id !== id);
    ...
    useEffect(() => {
        if (node) {
            ...
            setOrder(node.order);
            setPhotoPreview(node.photo_url ?? null);
        }
    }, [node]);

    const updateMutation = useUpdateOrgNodeMutation();
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let photoUrl = node?.photo_url ?? undefined;
        if (photoFile) {
            setUploading(true);
            try {
    ...
    if (isLoading) return <DashboardLayout><div className="flex items-center justify-center h-64 text-gray-400">Memuat data...</div></DashboardLayout>;
    if (!node) return <DashboardLayout><div className="flex items-center justify-center h-64 text-gray-400">Data tidak ditemukan.</div></DashboardLayout>;
            ...
            <div className="max-w-2xl mx-auto">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">{node.name} — {node.position}</p>
                    </div>
                </div>

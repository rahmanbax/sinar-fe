...
const TransactionToponymsPusatContent = () => {
    ...
    // Get region names from hooks based on the first item's IDs
    const firstProvinceId = toponyms.length > 0 ? (toponyms[0].province_id || toponyms[0].toponym?.province_id) : null;
    ...
    const mappedData = useMemo(() => {
        return toponyms.map((item: any, index: number) => {
            ...
            return {
                ...
                element_name: item.element?.name || item.element_name || item.toponym?.element?.name || item.toponym?.element_name || "-",
                nama_rupabumi: item.map_name || item.local_name || item.name || item.toponym?.map_name || item.toponym?.local_name || item.toponym?.name || "-",
                province_name: item.province_name || item.province?.name || item.toponym?.province_name || item.toponym?.province?.name || getProvinceName(item.province_id || item.toponym?.province_id),
                regency_name: item.regency_name || item.regency?.name || item.city_name || item.city?.name || item.toponym?.regency_name || item.toponym?.regency?.name || getRegencyName(item.regency_id || item.toponym?.regency_id),
                surveyor_name: item.creator?.name || item.surveyor_name || item.toponym?.creator?.name || item.toponym?.surveyor_name || "-",
            ...
            cell: (row) => (
                <div className="flex justify-center">
                    <button
                        onClick={() =>
                            router.push(
                                `/v2/verifikator-pusat/data-rekomendasi/detail/toponyms/${row.id}?transactionId=${transactionId}&rekomendasiId=${rekomendasiId}`
                            )
                        }
    ...
    return (
        <div className="flex flex-col gap-8">
            ...
            <div className="flex items-center gap-3">
                <button
                    onClick={() => router.push(`/v2/verifikator-pusat/data-rekomendasi/detail?id=${rekomendasiId}`)}
...
const TransactionToponymsPusatPage = () => {
    return (
        <DashboardLayout>
            <Suspense
                fallback={
            ...
            >
                <TransactionToponymsPusatContent />
            </Suspense>
        </DashboardLayout>
    );
};

export default TransactionToponymsPusatPage;

...
import { useAgendaDetail, useUpdateAgendaMutation } from "@/hooks/useCms";
...
const EditAgendaPage = () => {
    ...
    const { data: response, isLoading } = useAgendaDetail(token, id);
    ...
    useEffect(() => {
        if (agenda) {
            ...
            setDate(agenda.date.slice(0, 10));
            setPhotoPreview(agenda.photo_url ?? null);
        }
    }, [agenda]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        let photoUrl: string | undefined = agenda?.photo_url ?? undefined;
        ...
        updateMutation.mutate(
            { token, id, data: { title, date, photo_url: photoUrl } },
            {
    ...
    if (!agenda) {
        return (
            <DashboardLayout>
                <div className="flex items-center justify-center h-64 text-gray-400">Agenda tidak ditemukan.</div>
            </DashboardLayout>
        );
    }
            ...
            <div className="max-w-2xl mx-auto">
                <div className="flex items-center gap-3 mb-6">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500 truncate max-w-xs">{agenda.title}</p>
                    </div>
                </div>

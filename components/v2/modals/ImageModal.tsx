...
const ImageModal: React.FC<ImageModalProps> = ({ isOpen, onClose, images, initialSlideIndex = 0, altText }) => {
    ...
    const modalContent = (
        <div
            className={`fixed inset-0 z-9999 flex items-center justify-center p-4 sm:p-6 transition-all duration-300 ease-in-out ${isOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'
                }`}
        >
            {/* Backdrop */}
            <div
            ...
            {/* Modal Content */}
            <div
                className={`relative w-full h-[90vh] max-w-6xl flex flex-col items-center justify-center rounded-2xl overflow-hidden z-10 transition-transform duration-300 ease-in-out ${isOpen ? 'scale-100' : 'scale-95'
                    }`}
